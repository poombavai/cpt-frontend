		...
		function SaveKnowledgeBase()
		{
		  debugger; 
		  var projectid =localStorage.getItem('MyId');
		  var url= GlobalURl+"/move_to_knowledge_base?project_id="+projectid;
		  var xhr = new XMLHttpRequest();
                   xhr.open("GET", url, true);
                   xhr.onload = function (e) {
                  if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
					 ...
					 for(var resp in response)
					  {
					    debugger;
					     responsedvalue =response[resp];
						 
					  }
					  if(responsedvalue=='Some Taxonomies are yet to assign!')
					  {
					    myalertFunction('Info','Some Taxonomies are yet to assign!');
						
					  }
					  if(responsedvalue =='Successfully moved to Knowledge Base!')
					  {
					  	myalertFunction('Success','Successfully moved to Knowledge Base!');
					    var timeoutID = window.setTimeout(windowsuccesslocation, 3000);
		...
		function windowsuccesslocation()
		{
			 ...
			 if (e.deltaY > 0) {
			   ...
			   if(All==true)
			   {
				   if(count==checkval)
			       {
				     ...
				     count=0;
					 if(assandnonassicountval > 10)
					 {
						 checkval=8;
					 }
					  else if(assandnonassicountval > 5)
					 {
						 checkval=5;
					 }
					 else
